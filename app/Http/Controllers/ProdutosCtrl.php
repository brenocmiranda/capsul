    ...
    public function Lista(){
        return datatables()->of(Produtos::all())
                    ->editColumn('produto', function(Produtos $dados){ 
                        return '<div class="d-flex my-2"><div class="my-auto col"><img src="'. (isset($dados->RelationImagensPrincipal) ? asset('storage/app/'.$dados->RelationImagensPrincipal->first()->caminho) : asset('public/img/product.png')).'" alt="Imagem atual" style="height: 55px; width: 60px;" class="rounded" ></div><div class="col-9 text-left my-auto"><a href="'.route('produtos.editar', $dados->id).'" class="text-decoration-none"><p class="nome m-0">'.(strlen($dados->nome) <= 40 ? $dados->nome : substr($dados->nome, 0, 41)."...").'</p></a><label class="mb-0">'.$dados->cod_sku.'</label></div></div>';
